...
Feature: As an administrator, I want to be able to update the information
  ...
  Scenario:Verify a PATCH body containing valid authorization information and the correct (id) and correct
  data (title, category_id, description, content) is sent to the api/updateBlog/{id}
  endpoint, it must be verified that the returned status code is 200, the remark informationin
  the response body is "success" and the message information is "Successfully Updated."
  ...
  Scenario Outline: Verify that the blog is successfully updated with valid PATCH request

    * Rekare The api user constructs the base url with the "admin" token.
    # Api kullanicisi "admin" token ile base urli olusturur

    * Rekare The api user sets "api/updateBlog/<id>" path parameters.
    # Api kullanicisi "api/updateBlog/id" path parametrelerini olusturur

    * Rekare The api user prepares a PATCH request containing the "<title>" information to send to the api updateBlog endpoint.
    # Api kullanicisi api updateBlog endpointine gondermek icin "<title>" bilgisi iceren bir patch request body hazirlar
    ...
    * Rekare The api user verifies that the "Message" information in the response body is "Successfully Updated.".
    # Api kullanicisi response body'deki "Message" bilgisinin "Successfully Updated." oldugunu dogrular

    * Rekare The api user verifies that the "Updated Blog Id" information in the response body is the same as the id path parameter in the endpoint.
    # Api kullanicisi response body'deki "Updated Blog Id" bilgisinin endpointteki id path parametresi ile ayni oldugunu dogrular

    Examples:
      | id | title        |
      | 58 | Updated Test Title |
